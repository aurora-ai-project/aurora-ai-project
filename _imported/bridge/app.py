...
# JSON bundle with token, supports mkdir, write, append
@app.post("/apply")
...
def file_ops(op: str = Query(...), path: str = Query(...), text: str = Query("")):
    ...
    if op == "read":
        ...
        return {"status": "read", "path": str(tgt), "content": open(tgt).read()}
    if op == "delete":
        try:
            os.remove(tgt)
    ...
    if op == "mkdir":
        tgt.mkdir(parents=True, exist_ok=True)
        return {"status": "mkdir", "path": str(tgt)}
...
def public_apply_bundle(bundle: str, ts: int, sig: str):
    ...
    if not hmac.compare_digest(good, sig):
        raise HTTPException(status_code=401, detail="bad sig")
    spec = json.loads(_b64url_decode(bundle).decode())
    results = []
...
# Path based endpoints, base64url for content
...
@app.get("/path/mkdir/{filepath:path}")
def path_mkdir(filepath: str):
    tgt = _guard_path("/" + filepath)
    tgt.mkdir(parents=True, exist_ok=True)
    return {"status": "mkdir", "path": str(tgt)}
...
def path_list(filepath: str):
    ...
    return {"status": "ok", "path": str(base), "entries": items}

@app.get("/path/list-raw/{filepath:path}")
def path_list_raw(filepath: str):
    base = _guard_path("/" + filepath)
    if not base.exists() or not base.is_dir():
        raise HTTPException(status_code=404, detail="directory not found")
    rows = []
    for e in sorted(base.iterdir(), key=lambda p: (not p.is_dir(), p.name.lower())):
        ...
        rows.append(f"{kind:4}  {_hsize(info['size']):>6}  {info['mtime_iso']}  {e.name}")
